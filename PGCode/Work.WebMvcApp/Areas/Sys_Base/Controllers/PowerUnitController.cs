...
namespace DotWeb.Areas.Sys_Base.Controllers
{
    public class PowerUnitController : BaseController
    {
        ...
        {
            ViewBag.Unit_Option = MakeCollectDataToOptions(ac.CollectKeyValueData_Unit(),true);
        }

        [HttpPost]
        public String ajax_GetUnitPowerList(m_PowerUnit md)
        {
        ...
        [HttpPost]
        public String ajax_GetGridData(q_PowerUnit sh)
        {
            a_PowerUnit ac = new a_PowerUnit();
            ac.Connection = this.getSQLConnection();

            RunQueryEnd HResult = ac.SearchMaster(sh,LoginUserId);
