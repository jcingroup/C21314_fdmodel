...
{
    public class ModelDataController : BaseAction<m_ModelData, a_ModelData, q_ModelData, ModelData>
    {
        ...
        public override ActionResult ListGrid(q_ModelData sh)
        {
        ...
        public override ActionResult EditMasterNewData()
        {
            ...
            ac = new a_ModelData() { Connection = getSQLConnection(), logPlamInfo = plamInfo };
            md.id = ac.GetIDX();
            md.isopen = true;
        ...
        public override ActionResult EditMasterDataByID(int id)
        {
            ...
            RunOneDataEnd<m_ModelData> HResult = ac.GetDataMaster(id, LoginUserId);
        ...
        [HttpPost]
        [ValidateInput(false)]
        public String ajax_MasterUpdata(m_ModelData md)
        {
        ...
        /// <returns></returns>
        [HttpGet]
        public override String ajax_MasterGridData(q_ModelData queryObj)
        {
            #region 連接BusinessLogicLibary資料庫並取得資料
            ac = new a_ModelData() { Connection = getSQLConnection(), logPlamInfo = plamInfo };
            RunQueryPackage<m_ModelData> HResult = ac.SearchMaster(queryObj, LoginUserId);
            HandleResultCheck(HResult);
            #endregion
        ...
        public String ajax_UploadFine(int Id, String FilesKind, String hd_FileUp_EL)
        {
            ...
            try
            {
                ...
                {
                    if( FilesKind == "Main" )
                        HandImageSave(hd_FileUp_EL, Id, ImageFileUpParm.Main, FilesKind);
                    else
                        HandImageSave(hd_FileUp_EL, Id, ImageFileUpParm.Series, FilesKind);
                    rAjaxResult.result = true;
                    rAjaxResult.success = true;
                    rAjaxResult.FileName = hd_FileUp_EL.GetFileName();
                }
