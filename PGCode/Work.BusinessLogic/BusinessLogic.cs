...
namespace ProcCore.Business.Logic
{
    ...
    #region ModelData
    public class q_ModelData : QueryBase
    {
        public String s_sex { set; get; }
        public String s_division { set; get; }
        public Boolean? s_isopen { set; get; }
    }
    public class m_ModelData : ModuleBase
    {
        public int id { get; set; }
        public String name { get; set; }
    ...
    public class a_ModelData : LogicBase<m_ModelData, q_ModelData, ModelData>
    {
        public override RunInsertEnd InsertMaster(m_ModelData md, int accountId)
        {
            #region Variable declare area
            RunInsertEnd r = new RunInsertEnd(); //宣告回傳物件
            #endregion
            try
            {
                #region Main working
                Connection.BeginTransaction(); //開始交易鎖定
                TablePack<ModelData> dataWork = new TablePack<ModelData>(Connection) { LoginUserID = this.logPlamInfo.UserId, LoginUnitID = this.logPlamInfo.UnitId };
                ...
                dataWork.SetDataRowValue(x => x.id, md.id);
                dataWork.SetDataRowValue(x => x.name, md.name);
                dataWork.SetDataRowValue(x => x.height, md.height);
                dataWork.SetDataRowValue(x => x.bust, md.bust);
                dataWork.SetDataRowValue(x => x.waist, md.waist);
                dataWork.SetDataRowValue(x => x.hips, md.hips);
                dataWork.SetDataRowValue(x => x.shoes, md.shoes);
                dataWork.SetDataRowValue(x => x.hair, md.hair);
                dataWork.SetDataRowValue(x => x.eyes, md.eyes);
                dataWork.SetDataRowValue(x => x.sex, md.sex);
                dataWork.SetDataRowValue(x => x.division, md.division);
                dataWork.SetDataRowValue(x => x.sort, md.sort);
                dataWork.SetDataRowValue(x => x.isopen, md.isopen);
        ...
        public override RunUpdateEnd UpdateMaster(m_ModelData md, int accountId)
        {
            #region Variable declare area
            RunUpdateEnd r = new RunUpdateEnd();
            #endregion
            try
            {
                ...
                dataWork.TableModule.KeyFieldModules[dataWork.TableModule.id.N].V = md.id; //取得ID欄位的值
                m_ModelData md_Origin = dataWork.GetDataByKey<m_ModelData>(); //取得Key值的Data
        ...
        public override RunDeleteEnd DeleteMaster(Int32[] ids, int accountId)
        {
            ...
            try
            {
                ...
                Connection.BeginTransaction();
                //1、要刪除的資料先選出來
                TablePack<ModelData> dataWork = new TablePack<ModelData>(Connection) { LoginUserID = this.logPlamInfo.UserId, LoginUnitID = this.logPlamInfo.UnitId }; //宣告泛型物件並連接Connection
                dataWork.SelectFields(x => x.id); //只Select 主Key欄位
                dataWork.WhereFields(x => x.id, ids); //代入陣列Id值

                m_ModelData[] md_Origin = dataWork.DataByAdapter<m_ModelData>(); //取得Key值的Data
        ...
        public override RunQueryPackage<m_ModelData> SearchMaster(q_ModelData qr, int accountId)
        {
            #region 變數宣告
            RunQueryPackage<m_ModelData> r = new RunQueryPackage<m_ModelData>();
            #endregion
            try
            {
                #region Select Data 區段 By 條件
                #region 設定輸出至Grid欄位
                TablePack<ModelData> dataWork = new TablePack<ModelData>(Connection) { LoginUserID = this.logPlamInfo.UserId, LoginUnitID = this.logPlamInfo.UnitId };
                dataWork.SelectFields(x => new { x.id, x.name, x.height, x.bust, x.waist, x.hips, x.shoes, x.hair, x.eyes, x.sex, x.division, x.sort, x.isopen });
                ...
                #region 設定Where條件
                if (qr.s_division != null)
                    dataWork.WhereFields(x => x.division, qr.s_division, WhereCompareType.Like);
                if (qr.s_sex != null)
                    dataWork.WhereFields(x => x.sex, qr.s_sex, WhereCompareType.Like);
                if (qr.s_isopen != null)
                ...
                #region 輸出物件陣列
                r.SearchData = dataWork.DataByAdapter<m_ModelData>();
                r.Result = true;
                return r;
                #endregion
                #endregion
        ...
        public override RunOneDataEnd<m_ModelData> GetDataMaster(int id, int accountId)
        {
            #region 變數宣告
            RunOneDataEnd<m_ModelData> r = new RunOneDataEnd<m_ModelData>();
            ...
            try
            {
                ...
                TablePack<ModelData> dataWork = new TablePack<ModelData>(Connection) { LoginUserID = this.logPlamInfo.UserId, LoginUnitID = this.logPlamInfo.UnitId };
                dataWork.TableModule.KeyFieldModules[dataWork.TableModule.id.N].V = id; //設定KeyValue
                m_ModelData md = dataWork.GetDataByKey<m_ModelData>(); //取得Key該筆資料
